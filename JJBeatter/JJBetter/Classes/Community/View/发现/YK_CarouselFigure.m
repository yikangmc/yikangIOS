...
#import "YK_CarouselFigure.h"

@interface YK_CarouselFigure ()<UIScrollViewDelegate>
...
@implementation YK_CarouselFigure
...
//    _pageControl.frame = CGRectMake(LO_CFWidth - 20 * _pageControl.numberOfPages, LO_CFHeight + LO_CFY - 20 + height, 20 * _pageControl.numberOfPages, 20);
    _pageControl.frame = CGRectMake(0, 0, 20 * _pageControl.numberOfPages, 15 * WScale);
    _pageControl.center = CGPointMake(ScreenW / 2, 140 * WScale - 15 * WScale);    _pageControl.currentPage = 0;
...
{
    __block YK_CarouselFigure *men = self;
    self.block = ^(){
        if (block) {
            // 多张情况
