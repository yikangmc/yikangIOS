...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.backgroundColor = RGBA(51, 51, 51, 0.6);
        self.bgV = [[UIView alloc]initWithFrame:CGRectMake(0, ScreenH, ScreenW, 260 * WScale)];
...
- (void)creatDate{
    ...
    int current = [currentYear intValue];
...
//    [self.pickerV selectRow:[yearArray indexOfObject:currentYear]+81*50 inComponent:0 animated:YES];
    NSString *string = [NSString stringWithFormat:@"%@年",[formatter stringFromDate:date]];
    [self.pickerV selectRow:[yearArray indexOfObject:string] inComponent:0 animated:YES];
...
- (void)showAnimation{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        CGRect frame = self.bgV.frame;
        frame.origin.y = ScreenH - 260 * WScale - 64;
        self.bgV.frame = frame;
    }];
