...
@interface YKResetRecoverVC ()<UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,YKPickerViewDelegate>
...
@property (weak, nonatomic) IBOutlet UILabel *addressLabel; // 常驻地址
...
@implementation YKResetRecoverVC
...
- (void)setUserInfo
{
    ...
    [[HandleInformation sharedDataHandle] getMyInforWithBlock:^(NSString *isSuccess) {
        ...
        if ([isSuccess isEqualToString:@"加载个人信息成功"]) {
            ...
            [self.headImageView sd_setImageWithURL:[NSURL URLWithString:model.photoUrl] placeholderImage:[UIImage imageNamed:@"defaultHeaderImage_2"]];
            self.nameLabel.text = model.userName;
            if ([model.jobCategory integerValue] == 0) {
                self.workTypeLabel.text = @"兼职";
            }else if ([model.jobCategory integerValue] == 1) {
                self.workTypeLabel.text = @"全职";
            }else {
                self.workTypeLabel.text = @"请选择";
            }
